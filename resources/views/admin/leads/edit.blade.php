...
@section('header', 'Editar Lead: ' . $lead->name)
        ...
        <form action="{{ route('admin.leads.update', $lead) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="business" value="{{ old('business', $lead->business) }}"
                        class="w-full border rounded-lg px-3 py-2">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="source" class="w-full border rounded-lg px-3 py-2">
                    @foreach(['manual', 'web', 'agente', 'referido'] as $src)
                        <option value="{{ $src }}" {{ $lead->source === $src ? 'selected' : '' }}>{{ ucfirst($src) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="estimated_value" value="{{ old('estimated_value', $lead->estimated_value) }}" step="0.01" min="0"
            ...
            <div class="mb-6">
                ...
                <textarea name="project_description" rows="4" class="w-full border rounded-lg px-3 py-2">{{ old('project_description', $lead->project_description) }}</textarea>
            </div>

            <div class="flex gap-3">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">
                    <i class="fas fa-save mr-1"></i> Actualizar Lead
                </button>
                <a href="{{ route('admin.leads.show', $lead) }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300">Cancelar</a>
            </div>
        </form>
    </div>
</div>
