...
@section('header', 'Editar Servicio: ' . $service->name)
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.services.update', $service) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $service->name) }}" required class="w-full border rounded-lg px-3 py-2">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Slug (URL)</label>
                <input type="text" name="slug" value="{{ old('slug', $service->slug) }}" placeholder="se-genera-automaticamente" class="w-full border rounded-lg px-3 py-2">
                @if($service->public)
                    <p class="text-xs text-green-600 mt-1">URL de compra: {{ $service->publicUrl() }}</p>
                @else
                    <p class="text-xs text-gray-400 mt-1">Se usa en la URL de compra directa: /buy/slug</p>
                @endif
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                <textarea name="description" rows="3" class="w-full border rounded-lg px-3 py-2">{{ old('description', $service->description) }}</textarea>
            </div>
            <div class="mb-4">
                ...
                <input type="url" name="info_url" value="{{ old('info_url', $service->info_url) }}"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="price" value="{{ old('price', $service->price) }}" step="0.01" min="0" required class="w-full border rounded-lg px-3 py-2">
            </div>
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="active" value="1" {{ $service->active ? 'checked' : '' }} class="mr-2">
                    <span class="text-sm text-gray-700">Activo</span>
                </label>
            </div>
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="public" value="1" {{ $service->public ? 'checked' : '' }} class="mr-2">
                    <span class="text-sm text-gray-700">Público (compra directa desde tu sitio web)</span>
                </label>
            </div>
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="requires_domain" value="1" {{ $service->requires_domain ? 'checked' : '' }} class="mr-2">
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="email_service" value="1" {{ old('email_service', $service->email_service) ? 'checked' : '' }} class="mr-2">
            ...
            <div class="mb-6">
                ...
                <div class="flex gap-2">
                    <input type="text" name="twentyi_package_bundle_id" id="bundle-id-input"
                        value="{{ old('twentyi_package_bundle_id', $service->twentyi_package_bundle_id) }}"
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="sat_product_key"
                            value="{{ old('sat_product_key', $service->sat_product_key ?? '80101501') }}"
                    ...
                    <div>
                        ...
                        @php $taxObj = old('tax_object', $service->tax_object ?? '02'); @endphp
                        <select name="tax_object" class="w-full border rounded-lg px-3 py-2 text-sm">
                            <option value="02" {{ $taxObj === '02' ? 'selected' : '' }}>02 — Sí objeto de impuesto (normal)</option>
                            <option value="01" {{ $taxObj === '01' ? 'selected' : '' }}>01 — No objeto de impuesto</option>
                            <option value="03" {{ $taxObj === '03' ? 'selected' : '' }}>03 — Sí objeto, no obligado retención</option>
                        </select>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="sat_unit_key"
                            value="{{ old('sat_unit_key', $service->sat_unit_key ?? 'E48') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="sat_unit_name"
                            value="{{ old('sat_unit_name', $service->sat_unit_name ?? 'Servicio') }}"
                            placeholder="ej: Servicio" maxlength="50"
                            class="w-full border rounded-lg px-3 py-2 text-sm">
                    </div>
                </div>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="iva_exempt" value="1" {{ old('iva_exempt', $service->iva_exempt) ? 'checked' : '' }}>
            ...
            <div class="flex gap-3">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700"><i class="fas fa-save mr-1"></i> Actualizar</button>
