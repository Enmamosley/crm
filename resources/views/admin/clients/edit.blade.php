...
@section('header', 'Editar: ' . $client->legal_name)

@section('content')
<form action="{{ route('admin.clients.update', $client) }}" method="POST" class="max-w-3xl mx-auto space-y-6"
      x-data="{ billingType: '{{ old('billing_type', $client->billing_type ?? 'fiscal') }}' }">
    @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="tax_id" value="{{ old('tax_id', $client->tax_id) }}"
                    :required="billingType === 'fiscal'"
                    class="w-full border rounded-lg px-3 py-2 text-sm font-mono" style="text-transform:uppercase">
            </div>
            <div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <textarea name="notes" rows="3" class="w-full border rounded-lg px-3 py-2 text-sm">{{ old('notes', $client->notes) }}</textarea>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div x-data="{
            ...
            async createHosting() {
                ...
                try {
                    ...
                    if (json.success) {
                        ...
                        this.msgType = 'error';
                        this.msg = json.error || 'Error desconocido.';
                    }
                } catch (e) {
                    this.msgType = 'error';
            ...
            <p class="text-xs text-gray-400 mt-2">
                Ingresa el ID manualmente o usa el botón para crearlo automáticamente en tu cuenta 20i.
                <template x-if="!packageId && domain">
                    <span class="text-blue-500"> Se creará para: <code x-text="domain"></code>.</span>
                </template>
            </p>
        </div>
        ...
        <div class="mt-5 pt-4 border-t border-gray-100">
            ...
            <p class="text-xs text-gray-400 mt-2">
                Si el dominio está en Cosmotown, los name servers apuntan a 20i por defecto.
