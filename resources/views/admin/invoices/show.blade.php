...
@section('title', 'Factura ' . $invoice->folio())
...
@if($invoice->client && $invoice->client->portal_token && !$invoice->isPaid())
    <form action="{{ route('admin.invoices.send-link', $invoice) }}" method="POST" class="inline">
        @csrf
...
@elseif($invoice->status === 'draft' && !$invoice->isPaid())
    <form action="{{ route('admin.invoices.stamp', $invoice) }}" method="POST" class="inline"
...
@if($invoice->isStamped())
    <a href="{{ route('admin.invoices.pdf', $invoice) }}" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 text-sm">
        <i class="fas fa-file-pdf mr-1"></i> PDF
    </a>
    <a href="{{ route('admin.invoices.xml', $invoice) }}" class="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 text-sm">
...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    @if($invoice->status === 'cancelled')
        <p class="text-center text-sm text-red-500"><i class="fas fa-ban mr-1"></i>Factura cancelada ante el SAT</p>
    @else
    ...
    @if($invoice->quote)
        <p class="text-center text-xs text-gray-400 mt-2">
            Cotización: <a href="{{ route('admin.quotes.show', $invoice->quote) }}" class="text-blue-500 hover:underline">{{ $invoice->quote->quote_number }}</a>
        </p>
    @endif
    @endif
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                @php
                    ...
                    $bp = $invoice->billing_preference ?? 'fiscal';
                @endphp
                ...
                <div><span class="text-gray-500">Cotización:</span>
                    @if($invoice->quote)
                        <a href="{{ route('admin.quotes.show', $invoice->quote) }}" class="font-medium text-blue-600 hover:underline">{{ $invoice->quote->quote_number }}</a>
                    @else
                        <p class="font-medium text-gray-400">-</p>
                    @endif
                </div>
                @if($invoice->stamped_at)
                <div><span class="text-gray-500">Timbrada:</span><p class="font-medium">{{ $invoice->stamped_at->format('d/m/Y H:i') }}</p></div>
                @endif
                @if($invoice->cancelled_at)
                <div><span class="text-gray-500">Cancelada:</span><p class="font-medium text-red-600">{{ $invoice->cancelled_at->format('d/m/Y H:i') }}</p></div>
                @endif
            </div>
            @if($invoice->notes)
            ...
            <div class="mt-4 pt-4 border-t text-xs text-gray-400">
                FacturAPI ID: <span class="font-mono">{{ $invoice->facturapi_invoice_id }}</span>
            </div>
            @endif
        </div>
            ...
            <table class="w-full">
                ...
                <tfoot class="bg-gray-50">
                    <tr><td colspan="3" class="px-6 py-2 text-right text-sm text-gray-500">Subtotal</td>
                        <td class="px-6 py-2 text-right text-sm">${{ number_format($invoice->subtotal, 2) }}</td></tr>
                    <tr><td colspan="3" class="px-6 py-2 text-right text-sm text-gray-500">IVA</td>
                        <td class="px-6 py-2 text-right text-sm">${{ number_format($invoice->iva_amount, 2) }}</td></tr>
                    <tr><td colspan="3" class="px-6 py-3 text-right font-bold">Total</td>
                        <td class="px-6 py-3 text-right font-bold text-lg">${{ number_format($invoice->total, 2) }}</td></tr>
                </tfoot>
            </table>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="text-sm text-gray-400 text-center py-6">
                ...
                @if($invoice->client && $invoice->client->portal_token)
                <br><a href="{{ route('portal.checkout', [$invoice->client->portal_token, $invoice]) }}"
    ...
    <div>
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-2 text-sm">
                ...
                <p class="text-gray-500 text-xs mt-2">CP {{ $invoice->client->address_zip }}, {{ $invoice->client->address_city }}</p>
            </div>
            <a href="{{ route('admin.clients.show', $invoice->client) }}" class="mt-3 inline-block text-sm text-blue-600 hover:underline">Ver cliente →</a>
            @endif
        </div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4 text-sm text-yellow-700">
            ...
            <a href="{{ route('admin.clients.show', $invoice->client) }}" class="underline">Activarlo →</a>
        </div>
        @endif
...
<div id="manualPayModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
        ...
        <form action="{{ route('admin.invoices.pay-manual', $invoice) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
