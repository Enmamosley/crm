...
# Pagos con Mercado Pago (Checkout API)

## Descripción general

El CRM integra [Mercado Pago Checkout API](https://www.mercadopago.com.mx/developers/es/docs/checkout-api-orders/overview) para cobrar facturas directamente desde el **portal del cliente**, sin redirección. Se soportan tres métodos de pago:

| Método | Tipo | Experiencia |
|---|---|---|
| Tarjeta crédito/débito | Online | Pago inmediato con tokenización PCI (MercadoPago.js) |
| OXXO | Efectivo | Genera referencia/código de barras, hasta 3 días para pagar |
| SPEI | Transferencia | Genera CLABE interbancaria, confirmación en minutos |

## Configuración requerida

| Setting (tabla `settings`) | Descripción |
|---|---|
| `mp_public_key` | Clave pública para MercadoPago.js en el frontend (`APP_USR-...` o `TEST-...`) |
| `mp_access_token` | Token privado para llamadas server-side (`APP_USR-...` o `TEST-...`) |
| `mp_webhook_secret` | *(Opcional)* Secreto para validar firma HMAC de webhooks |

Se configuran en **Admin → Configuración → Mercado Pago**.

## Flujo de pago

```
1. Admin crea factura desde cotización aceptada
2. Cliente entra al portal → ve factura con botón "Pagar"
3. Portal muestra checkout (tarjeta, OXXO o SPEI)
4. Cliente elige método y paga
5. Para tarjeta: MercadoPago.js tokeniza → backend procesa → resultado inmediato
6. Para OXXO/SPEI: se genera referencia → cliente paga externamente
7. Webhook de MP notifica al CRM al confirmar el pago
8. CRM actualiza Payment + marca factura como pagada (paid_at)
```
...
| Método | Descripción |
|---|---|
| `createCardPayment(invoice, token, paymentMethodId, email, installments?, issuerId?)` | Crea pago con tarjeta tokenizada |
| `createOxxoPayment(invoice, email)` | Genera referencia OXXO |
| `createSpeiPayment(invoice, email)` | Genera referencia SPEI |
| `getPayment(mpPaymentId): ?array` | Consulta estado del pago en API de MP |
| `syncPaymentStatus(payment): Payment` | Sincroniza estado desde MP y marca factura como pagada si aplica |
| `validateWebhookSignature(xSignature, xRequestId, dataId): bool` | Valida firma HMAC del webhook |
...
## API de Mercado Pago — Endpoints usados
...
```json
{
  ...
  "token": "card_token_from_js",
  "description": "Factura F001 – Empresa SA",
  "installments": 1,
  "payment_method_id": "visa",
  "payer": { "email": "cliente@email.com" },
  "external_reference": "invoice_42"
}
```
...
Validación HMAC (`x-signature` header): `ts=TIMESTAMP,v1=HMAC_SHA256("id:{data.id};request-id:{x-request-id};ts:{ts};", secret)`
...
|---|---|---|
| `client_invoice_id` | FK | Factura asociada |
| `mp_payment_id` | string | ID del pago en Mercado Pago |
| `amount` | decimal(12,2) | Monto cobrado |
| `currency` | string(3) | Moneda (default: MXN) |
| `status` | enum | `pending`, `approved`, `in_process`, `rejected`, `refunded`, `cancelled` |
| `status_detail` | string | Detalle del rechazo (ej: `cc_rejected_insufficient_amount`) |
| `payment_type` | string | `credit_card`, `debit_card`, `ticket`, `bank_transfer` |
| `payment_method_id` | string | `visa`, `master`, `oxxo`, `bank_transfer`, etc. |
| `mp_data` | JSON | Respuesta completa de la API de MP |
| `paid_at` | timestamp | Fecha/hora de aprobación |

Helpers: `isPending()`, `isApproved()`, `isRejected()`, `satPaymentForm()`, `ticketUrl()`.

### Mapeo pago MP → forma de pago SAT

| `payment_type` MP | Código SAT | Descripción |
|---|---|---|
...
## Rutas — Portal del cliente (pago)

| Método | URI | Nombre | Descripción |
|---|---|---|---|
| GET | `portal/{token}/invoices/{invoice}/checkout` | `portal.checkout` | Página de checkout |
| POST | `portal/{token}/invoices/{invoice}/pay/card` | `portal.pay.card` | Procesar pago con tarjeta (AJAX/JSON) |
| POST | `portal/{token}/invoices/{invoice}/pay/oxxo` | `portal.pay.oxxo` | Generar referencia OXXO |
| POST | `portal/{token}/invoices/{invoice}/pay/spei` | `portal.pay.spei` | Generar referencia SPEI |
| GET | `portal/{token}/payments/{payment}` | `portal.payment.status` | Ver estado del pago |

## Ruta — Webhook API

| Método | URI | Nombre | Descripción |
|---|---|---|---|
| POST | `api/webhooks/mercadopago` | `mercadopago.webhook` | Recibe notificaciones de MP |

## Vistas

| Vista | Descripción |
|---|---|
| `portal/checkout.blade.php` | Checkout con tabs (Tarjeta/OXXO/SPEI), incluye MercadoPago.js SDK |
| `portal/payment-status.blade.php` | Estado del pago: aprobado, rechazado, pendiente con ticket/CLABE |
| `portal/dashboard.blade.php` | Botón "Pagar" en facturas no pagadas (visible solo si `mp_public_key` está configurada) |
| `admin/settings/index.blade.php` | Sección de configuración de credenciales MP |

## Notas de implementación

- **Idempotencia:** Cada llamada a `/v1/payments` incluye header `X-Idempotency-Key` (`inv_{id}_{timestamp}`) para prevenir cobros duplicados.
- **PCI Compliance:** Los datos de tarjeta nunca tocan el servidor. MercadoPago.js crea iframes seguros que tokenizan los datos directamente con MP.
- **Verificación por API:** El webhook siempre verifica el estado real del pago llamando a `GET /v1/payments/{id}` — nunca confía solo en el payload del webhook.
- **Campo `paid_at`:** Se agregó a `client_invoices` para marcar facturas como pagadas. Se actualiza automáticamente cuando MP confirma el pago.
- **Sin redirección:** Todo el flujo ocurre dentro del portal del cliente. La experiencia es nativa en el sitio.
- **Meses sin intereses:** El SDK de MP pobla automáticamente las opciones de MSI basadas en el BIN de la tarjeta y el monto.
